...
import { insertBefore, removeElement } from '@cnbritain/merlin-www-js-utils/js/functions';
...
function hasConsent(consentType){
  if(consentType === 'STRICTLY' && OneTrustManager.consentedStrictlyCookies){
    return true;
  } else if(consentType === 'PERFORMANCE' && OneTrustManager.consentedPerformanceCookies){
    return true;
  } else if(consentType === 'FUNCTIONAL' && OneTrustManager.consentedFunctionalCookies){
    return true;
  } else if(consentType === 'TARGETING' && OneTrustManager.consentedTargetingCookies){
    return true;
  } else if(consentType === 'SOCIAL_MEDIA' && OneTrustManager.consentedSocialNetworkCookies){
    return true;
  } else {
    return false;
  }
...
function hydrate(){
  ...
  for(var i = 0; i < templates.length; i++){
    ...
    if(!isHydrated(template) && hasConsent(template.parentNode.getAttribute('data-consent-type'))){
